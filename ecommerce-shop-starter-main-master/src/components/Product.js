...
const Product = ({product}) => {
  ...
  return(
      ...
      opacity-0 group-hover:opacity-100 transition-all duration-300'>
        <button onClick={()=>addToCart(product, id)} >
          <div className='flex justify-center items-center text-white w-12 h-12 bg-red-500'>
            <BsPlus className='text-3x1 w-12 h-12'/>
          </div>
        </button>
        <Link to={`/product/${id}`} 
        className='w-12 h-21 bg-white flex 
        justify-center text-primary drop-shadow-x1'>
          <BsEyeFill/>
        </Link>
      </div>
    </div>
    ...
    <div>
      ...
      <Link to={`/product/${id}`}>
        <h2 className='font-semi-bold mb-1'>{title}</h2>
      </Link>
